...
int main() {
	...
	// --------------------
	GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Spotlight", NULL, NULL);
	if (window == NULL) {
	...
	unsigned char *texData = stbi_load("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/diffuse.png", &texWidth, &texHeight, &texChannel, 0);
	if (texChannel == 1)
		format = GL_RED;
	...
	if (texData) {
		...
		std::cout << "failed to load image diffuse.png" << std::endl;
		return -1;
	}
	stbi_image_free(texData);
	//载入第二个纹理

	unsigned char *texData2 = stbi_load("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/specular.png", &texWidth, &texHeight, &texChannel, 0);
	if (texChannel == 1)
		format = GL_RED;
	else if (texChannel == 3)
		format = GL_RGB;
	...
	if (texData2) {
		...
		std::cout << "failed to load image specular.png" << std::endl;
		return -1;
	}
	...
	//配置盒子
	// ------------------------------------
	const char *vertexShaderSource = readShader("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/boxVertex.glsl");
	const char *fragmentShaderSource = readShader("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/boxFragment.glsl");
	// vertex shader
	...
	//光源设置开始
	//光源设置开始
	const char *lightVertexShaderSource = readShader("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/lightVertex.glsl");
	const char *lightFragmentShaderSource = readShader("/Users/hezhenbang/Documents/OpenGL Projects/Spotlight/cmake-build-debug/lightFragment.glsl");
	// vertex shader
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for(int i=0;i<5;i++) {
			...
			//聚光灯设置
			glUniform1f(glGetUniformLocation(shaderProgram, "light.linear"), 0.014);
			glUniform1f(glGetUniformLocation(shaderProgram, "light.quadratic"), 0.007);
			glUniform1f(glGetUniformLocation(shaderProgram, "light.innerCos"), glm::cos(glm::radians(10.0f)));
			glUniform1f(glGetUniformLocation(shaderProgram, "light.outerCos"), glm::cos(glm::radians(14.0f)));
			glUniform3f(glGetUniformLocation(shaderProgram, "light.spotDirection"), cameraFront.x, cameraFront.y, cameraFront.z);
