...
struct Texture {
	...
	aiString path;
	Texture(){}
	Texture(GLuint i,std::string t):id(i),type(t){};
};
...
void Mesh::draw(Shader shader) {
	...
	for (int i = 0; i < textures.size(); ++i) {
		...
		std::string number;
		if ("diffuse" == name)ss << diffuseNum++;
		else if ("specular" == name)ss << specularNum++;
		else if ("normal" == name)ss << normalNum++;
		else if ("height" == name)ss << heightNum++;
		number = ss.str();
